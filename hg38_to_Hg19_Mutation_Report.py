...
#a directory named according to the import date of the proband file will be created to hold Mutation Reports generated by this script, extract import date from filename
...
#assign each of the gel ids collected to a gel_id variable so a csv file can be created for each of them
for index, row in df_gel.iterrows():
	...
	#some gel ids return empty SNP csv files - delete files and add gel ids to an empty.txt file for review
...
#the empty_files.txt file now holds a list of empty SV and SNP gel ids - therefore, truly empty gel ids are in twice and need the duplicate removing - those in once are not truely empty as they have one type of variant - therefore, these also need removing 
def emptyfiles():
...
def lift_over(input_file_list):
	...
	for input_file in input_file_list:
		...
		for index, row in df.iterrows():
			...
			#perform liftover of build 38 coordinates to build 37 (not mitrocondrial genome or those already build 37)
			...
			#do not perform liftover on coordinates already in build 37 or for mitochondrial genome, instead append the original position to Hg19
			else:
...
#occassionally a conversion is not available - need to catch this so it does not error downstream
def conversion_blanks(input_file_list):
	...
	for input_file in input_file_list:
		...
		#convert Hg19 to string so the '.find' method can be applied
		df['Hg19'] = df['Hg19'].astype(str)

		#iterate over rows of data frame 
		for index, row in df.iterrows():

			#if the build was 38 and it was not a mitocondrial genome, it should have been converted and therefore needs checking, iterate through
			if row[9] == 'GRCh38' and row[4] != 'chrM':

				#all conversions contain the string 'chr', can therefore use this to check a conversion has occured
				if row['Hg19'].find('chr') > 0:
					pass

				#if the conversion has failed, add the gel id and genomic position to a file named 'conversion_unavailable.txt'
				else:
					with open('conversion_unavailable.txt', 'a') as f:
						f.write(input_file + ', ' + 'Position: ' + str(row[0]) + '\n')

			#no need to check if the build was already 37 or it was a mitocondrial genome as there is no conversion
			else:
				pass
	
	#if a csv file has been identified as having a converion error, delete it		
	if os.path.exists('conversion_unavailable.txt'):
...
#reassign each of the csv files to the variable so script doesn't search for deleted files
input_file_list = glob.glob('Gel2MDT_Export_*_MutationReport.csv')
...
def extract_genomic_coord(input_file_list):
	...
	#read in batch of csv files as pandas dataframe
	for input_file in input_file_list:
		df = pd.read_csv(input_file, header=0, names= ['hg38 Reference Position', 'Gene', 'Reference Sequence', 'Alternative Sequence', 'Chr', 'Genotype', 'Genomic Coordinate', 'Alamut', 'Tier', 'Reference Genome', 'Reference Position'])
...
def lift_over_genomic_coord(input_file_list):
	...
	#read in batch of csv files as pandas dataframe
	for input_file in input_file_list:
		df = pd.read_csv(input_file, header=0, names= ['hg38 Reference Position', 'Gene', 'Reference Sequence', 'Alternative Sequence', 'Chr', 'Genotype', 'Genomic Coordinate', 'Alamut', 'Tier', 'Reference Genome', 'Reference Position', 'First Coordinate', 'Second Coordinate'])
...
def conversion_blanks_2(input_file_list):
	...
	for input_file in input_file_list:
		...
		#not all variants have a second coordinate so fill blanks with the word 'empty' for easy searching  
		df['Second Coordinate'] = df['Second Coordinate'].fillna('empty')

		#change the coordinate columns to strings so the '.find' method can be used on them 
		...
		for index, row in df.iterrows():
			...
			#as above with build 38 and the mitochondrial genome. If the second coordinate is not empty, a conversion has been attemped and it needs to be checked 
			if row[9] == 'GRCh38' and row[4] != 'M' and row[12] != 'empty':

				#if a conversion has successfully been carried out, the field will contain 'chr' and can be passed 
				if row['First Coordinate LiftOver'].find('chr') > 0 and row['Second Coordinate LiftOver'].find('chr') > 0:
					pass

				#if not, the conversion has failed and the gel id and position should be added to the 'conversion_anavailable.txt' file 
				else:
					with open('conversion_unavailable.txt', 'a') as f:
						f.write(input_file + ', ' + 'Position: ' + str(row[0]) + '\n')

			#as above, but if the second coordinate field is 'empty'; there was no second coordinate for conversion, therefore only the first coodinate should be checked 
			...
			#37 and mito builds have also not been converted and therefore do not require checking
			else:
				pass
	
	#if the conversion has failed, delete the respective csv file
	if os.path.exists('conversion_unavailable.txt'):
		with open('conversion_unavailable.txt', 'r') as f:
			to_delete = f.read().splitlines()
...
#reassign each of the csv files so the script is not searching for deleted files 
input_file_list = glob.glob('Gel2MDT_Export_*_MutationReport.csv')
...
def update_genomic_coord(input_file_list):
	...
	#read in batch of csv files as pandas dataframe
	for input_file in input_file_list:
		df = pd.read_csv(input_file, header=0, names= ['hg38 Reference Position', 'Gene', 'Reference Sequence', 'Alternative Sequence', 'Chr', 'Genotype', 'Genomic Coordinate', 'Alamut', 'Tier', 'Reference Genome', 'Reference Position', 'First Coordinate', 'Second Coordinate', 'First Reference Position', 'Second Reference Position'])
...
def update_alamut_coord(input_file_list):
	...
	#read in batch of csv files as pandas dataframe
	for input_file in input_file_list:
		df = pd.read_csv(input_file, header=0, names= ['hg38 Reference Position', 'Gene', 'Reference Sequence', 'Alternative Sequence', 'Chr', 'Genotype', 'Genomic Coordinate', 'Alamut', 'Tier', 'Reference Genome', 'Reference Position', 'Concat Reference Positions', 'Genomic Change'])
...
def combine_genes(input_file_list_SV):
	...
	for input_file in input_file_list_SV:
		df = pd.read_csv(input_file, header=0, names= ['Start', 'End', 'Gene', 'Chr', 'Tier', 'Reference Genome', 'Variant Type'])
...
def conversion_blanks_SV(input_file_list_SV):
	...
	for input_file in input_file_list_SV:
		...
		#iterate over rows of dataframe
		for index, row in df.iterrows():
			#check conversion by assessing whether the values contain 'chr'
			if row[4] == 'GRCh38' and row[2] != 'chrM':
				if row['start_Hg19'].find('chr') > 0 and row['end_Hg19'].find('chr') > 0:
					pass
				#if there was a problem with conversion, add the gel id and position to 'conversion_unavailable.txt' file
				else:
					with open('conversion_unavailable.txt', 'a') as f:
						f.write(input_file + ', ' + 'Position: ' + str(row[0]) + '-' + str(row[1]) + '\n')
			#build 37 positions and mitochondiral genome don't need checking as no converion occured
			else:
				pass
			
	#delete files in which a conversion was unavailble to prevent problems downstream
	if os.path.exists('conversion_unavailable.txt'):
		with open('conversion_unavailable.txt', 'r') as f:
			to_delete = f.read().splitlines()
...
#merge the SNP and SV files into one Mutation Report
def merge():

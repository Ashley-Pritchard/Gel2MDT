...
#ask for input of import date into the Gel2MDT database via the command line 
update_time = input("Please input update date in format yyyy-mm-dd: ")

#check that the user has input the date in the correct format. Error if not. 
def validate(update_time):
	try:
...
#create a csv file with a list of gel ids uploaded on the date input by the user from the Gel2MDT database.
def get_list(update_time):
	...
	#save the gel ids to a csv file
	rows = cur.fetchall()
	filename = 'gel_id.csv'
...
if df_gel.empty:
	...
	print('Gel ids have been lifted from the Gel2MDT database for this date. A csv file for each will now be created.')
